...
class ConstraintGenerator:
    r"""Utility class for generating and managing constraint code."""

    def __init__(self, config: GenerationConfig):
        self.llm = LLMGenerator(config)
        self.template = (Path(__file__).parent / "template.txt").read_text()

    def generate_constraint_code(self, problem_statement: str, solution: str) -> str:
    ...
    def batch_generate_constraint_code(
        self,
    ...
    def batch_generate_constraint_code_traced(
        self,
        problem_statements: dict[str, str],
        solutions: dict[str, str],
        max_workers: int = 16,
        ...
        for problem_id, trace in traces.items():
            ...
            try:
                ...
                _save_trace_artifacts("stage1", problem_id, trace, code, output_dir)
    ...
    def save_constraint_code(self, code: str, output_path: str) -> str:
    ...
    def batch_save_constraint_code(
        self,
        codes: dict[str, str],
        output_dir: str,
        filename_template: str = "{problem_id}_constraint.py",
    ) -> dict[str, str]:
