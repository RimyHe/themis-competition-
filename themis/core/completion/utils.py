...
class CompletionDetails:
    r"""Raw completion result and request metadata."""
...
class OpenAICompletion:
    ...
    async def completion(
        ...
        details = await self.completion_details(
            is_chat=is_chat,
            prompt=prompt,
            model=model,
        ...
        if details.error:
            logger.error(f"API call failed: {details.error}")
        return details.content

    async def completion_details(
        self,
        is_chat: bool,
        prompt: str | list[dict[str, str]],
        model: str = "Qwen/Qwen2.5-Coder-7B-Instruct",
        ...
        r"""Execute a completion request and preserve raw response metadata."""
        if stop is None:
        ...
        try:
            ...
            return CompletionDetails(content="", finish_reason=None, error=str(e))
